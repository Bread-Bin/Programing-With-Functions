...
def print_death_age(people_dict):
    for person in people_dict:
        ...
        print(f"{name} died at age {age_at_death}, born in {birth_year}, died in {death_year}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for wedding in marriages_dict:
        marriage_year = marriages_dict[wedding][WEDDING_YEAR_INDEX]
        husband_code = marriages_dict[wedding][HUSBAND_KEY_INDEX]
        wife_code = marriages_dict[wedding][WIFE_KEY_INDEX]
        for person in people_dict:
            husband_name = people_dict[husband_code][NAME_INDEX]
            husband_birth_year = people_dict[husband_code][BIRTH_YEAR_INDEX]
            husband_wedding_age = marriage_year - husband_birth_year

            wife_name = people_dict[wife_code][NAME_INDEX]
            wife_birth_year = people_dict[wife_code][BIRTH_YEAR_INDEX]
            wife_wedding_age = marriage_year - wife_birth_year
        print(f'{husband_name} {husband_wedding_age} > {marriage_year} < {wife_name} {wife_wedding_age}')
...
def count_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        people_dict[husband_key][NUM_MARRIAGES_INDEX] += 1
        wife_list = people_dict[wife_key]
        people_dict[wife_key][NUM_MARRIAGES_INDEX] += 1

    for person_key, person_list in people_dict.items():
        name = person_list[NAME_INDEX]
        num_marriages = person_list[NUM_MARRIAGES_INDEX]
        print(f'{name} was married {num_marriages} times.')
